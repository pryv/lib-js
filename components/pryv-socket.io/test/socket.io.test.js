  ...
  describe('[SNTX] notification', () => {
    ...
    it('[SNTA] Fails on requesting an invalid notifcation', async () => {
      try {
        ...
        return expect(e.message).to.equal('Unkown event [Bogus]. Allowed events are: eventsChanged,streamsChanged,accessesChanged,accessUpdated,notificationsChanged,disconnect,error');
      }
